...
namespace CompanyEmployees.Utilities
{
    public class EmployeeLinks
    {
            ...
            HttpContext httpContent)
        {
            ...
            if (ShouldGenerateLinks(httpContent))
            {
                return ReturnLinkdedEmployees(employeeDto, fields, companyId, httpContent, shapedEmployees);
            }
        ...
        {
            return _dataShaper.ShapeData(employeeDto, fields).Select(x => x.Entity).ToList();
        }
